...
const Icon: React.FC<IconProps> = ({ src, alt, label }) => {
  ...
  return (
    ...
    >
      ...
      {/* 아이콘 이름 */}
      <motion.p
        className="text-sm font-semibold mt-2 text-center text-black"
        initial={{ opacity: 0, y: 10 }}
        animate={
          isTooltipVisible ? { opacity: 1, y: 0 } : { opacity: 1, y: 10 }
        } // 마우스 오버 시 보이도록 설정
      ...
      {/* 툴팁 */}
      {isTooltipVisible && (
        <div className="absolute bottom-16 bg-black text-white text-xs rounded px-2 py-1">
          {label}
        </div>
      )}
    </div>
...
interface BucketProps {
  id: string;
  label: string;
  skills: { src: string; alt: string; label: string }[]; // 아이콘 목록
}
    ...
    >
      ...
      >
        <div className="content flex flex-col items-center justify-center">
          ...
          {isHovered && <SkillsList skills={skills} />} {/* 아이콘 목록 표시 */}
        </div>
      </motion.div>
    </div>
  );
...
const SkillsSection: React.FC = () => {
  ...
  const cooperationSkills = [
    ...
    {
      src: "/images/icons/Skills/notion.svg",
      alt: "Notion",
      label: "Notion",
    },
  ];
